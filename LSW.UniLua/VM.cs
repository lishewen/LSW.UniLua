...
namespace LSW.UniLua
{
	...
	public partial class LuaState
	{
		...
		private bool V_ToNumber(StkId obj, ref TValue n)
		{
			...
			if (obj.V.TtIsString())
			{
				if (O_Str2Decimal(obj.V.SValue(), out double val))
				{
					n.SetNValue(val);
					return true;
				}
		...
		private bool V_LessThan(StkId lhs, StkId rhs)
		{
			...
			var res = CallOrderTM(lhs, rhs, TMS.TM_LT, out bool error);
			if (error)
			{
		...
		private bool V_LessEqual(StkId lhs, StkId rhs)
		{
			...
			// first try `le'
			var res = CallOrderTM(lhs, rhs, TMS.TM_LE, out bool error);
			if (!error)
				return res;
